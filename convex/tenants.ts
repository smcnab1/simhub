...
const platformProvisioningRoleValidator = v.union(
  v.literal("Developer"),
  v.literal("Admin"),
  v.literal("Staff"),
  v.literal("Requester")
...
export const createPlatformTenant = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (payload.workosOrganizationId) {
      const existingByOrg = await ctx.db
        .query("tenants")
        .withIndex("by_workos_org", (q) =>
          q.eq("workosOrganizationId", payload.workosOrganizationId)
        )
        .unique();

      if (existingByOrg) {
        throw new Error("That WorkOS organization is already linked to another tenant.");
      }
    }
    ...
    if (payload.customDomain) {
      const existingByDomain = await ctx.db
        .query("tenants")
        .withIndex("by_custom_domain", (q) =>
          q.eq("customDomain", payload.customDomain)
        )
        .unique();

      if (existingByDomain) {
        throw new Error("That custom domain is already linked to another tenant.");
      }
    }
...
export const upsertPlatformTenantShell = mutation({
  args: {
    auth: authContextValidator,
    tenant: v.object(platformTenantFields),
  },
  handler: async (ctx, args) => {
    ...
    if (payload.workosOrganizationId) {
      ...
      if (existingByOrg && existingByOrg._id !== existingBySlug?._id) {
        throw new Error("That WorkOS organization is already linked to another tenant.");
      }
    }
    ...
    if (payload.customDomain) {
      ...
      if (existingByDomain && existingByDomain._id !== existingBySlug?._id) {
        throw new Error("That custom domain is already linked to another tenant.");
      }
    }

    if (existingBySlug) {
      await ctx.db.patch(existingBySlug._id, payload);
      return {
        tenantId: existingBySlug._id,
        slug: payload.slug,
        name: payload.name,
...
export const rollbackPlatformTenantShell = mutation({
  args: {
    auth: authContextValidator,
  ...
  handler: async (ctx, args) => {
    ...
    if (users.length) {
      await ctx.db.patch(args.tenantId, { active: false });
      return { rolledBack: "deactivated" };
    }
...
export const finishPlatformTenantProvisioning = mutation({
  args: {
    auth: authContextValidator,
    tenantId: v.id("tenants"),
    ...
    user: v.optional(
      v.object({
        ...
        invitationState: v.optional(v.string()),
        email: v.string(),
        name: v.string(),
        role: platformProvisioningRoleValidator,
        workosMembershipId: v.optional(v.string()),
      })
    ),
    workosOrgCreated: v.boolean(),
    workosUserCreated: v.boolean(),
    workosInvitationCreated: v.boolean(),
    workosMembershipChanged: v.boolean(),
  },
  handler: async (ctx, args) => {
    requirePlatformDeveloper(ctx, args.auth);
    ...
    if (orgTenant && orgTenant._id !== tenant._id) {
      throw new Error("That WorkOS organization is already linked to another tenant.");
    }

    const wasMissingOrg = !tenant.workosOrganizationId;
    await ctx.db.patch(tenant._id, {
      workosOrganizationId: args.workosOrganizationId,
      active: true,
    });

    if (wasMissingOrg || args.workosOrgCreated) {
      await recordAuditEvent(ctx, {
        ...
        entityId: tenant._id,
        actor: {
          name: args.auth.user?.firstName,
          email: args.auth.email ?? args.auth.user?.email,
        },
        visibility: "developer",
        message: `Tenant ${tenant.name} was provisioned.`,
        metadata: {
          tenantSlug: tenant.slug,
          workosOrganizationId: args.workosOrganizationId,
          workosOrgCreated: args.workosOrgCreated,
        },
      });
    }

    if (!args.user) {
      return { tenantId: tenant._id, userId: null };
    }

    const provisionedUser = args.user;
    const email = provisionedUser.email.trim().toLowerCase();
    assertValidEmail(email, "Primary admin email");
    ...
    const existingByEmail = await ctx.db
      ...
      .unique();
    const existingByWorkOS =
      provisionedUser.workosUserId
        ? (
            await ctx.db
              .query("users")
              .withIndex("by_workos_user", (q) =>
                q.eq("workosUserId", provisionedUser.workosUserId!)
              )
              .collect()
          ).find((user) => user.tenantId === tenant._id)
        : null;
    const existing = existingByEmail ?? existingByWorkOS ?? null;
    const payload = {
      tenantId: tenant._id,
      workosUserId: provisionedUser.workosUserId ?? `email:${email}`,
      email,
      name: provisionedUser.name.trim() || email,
    ...
    if (!existing || args.workosUserCreated || args.workosInvitationCreated) {
      await recordAuditEvent(ctx, {
        tenantId: tenant._id,
        eventType: args.workosInvitationCreated ? "user.invited" : "user.created",
        entityType: "user",
        entityId: userId,
        actor: {
          name: args.auth.user?.firstName,
        ...
        visibility: "developer",
        message: `${payload.email} was ${args.workosInvitationCreated ? "invited" : "created"}.`,
        metadata: {
          workosUserId: provisionedUser.workosUserId,
          invitationId: provisionedUser.invitationId,
          invitationState: provisionedUser.invitationState,
        },
      });
    }
    ...
    await recordAuditEvent(ctx, {
      ...
      visibility: "developer",
      message: `${payload.email} was assigned ${payload.role} access to ${tenant.name}.`,
      metadata: {
        workosOrganizationId: args.workosOrganizationId,
        workosMembershipId: provisionedUser.workosMembershipId,
        workosMembershipChanged: args.workosMembershipChanged,
      },
    });

    return { tenantId: tenant._id, userId };
  },
});
...
export const syncAuthenticatedUser = mutation({
  args: { auth: authContextValidator },
  handler: async (ctx, args) => {
    const workosUserId = args.auth.workosUserId?.trim() || args.auth.user?.id?.trim();
    const email = args.auth.email?.trim().toLowerCase() || args.auth.user?.email?.trim().toLowerCase();
    ...
    for (const tenant of tenants) {
      ...
      if (!byEmail || byEmail.workosUserId === workosUserId) {
        continue;
      }

      if (
        byEmail.workosUserId.startsWith("email:") ||
        byEmail.workosUserId.trim() === ""
      ) {
        await ctx.db.patch(byEmail._id, {
          workosUserId,
          name:
            [
              args.auth.user?.firstName?.trim(),
              args.auth.user?.lastName?.trim(),
            ]
              .filter(Boolean)
              .join(" ") || byEmail.name,
        });
...
export const updatePlatformTenant = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingBySlug && existingBySlug._id !== args.tenantId) {
      throw new Error("A tenant with this slug already exists.");
    }
    ...
    if (payload.workosOrganizationId) {
      ...
      if (existingByOrg && existingByOrg._id !== args.tenantId) {
        throw new Error("That WorkOS organization is already linked to another tenant.");
      }
    }
    ...
    if (payload.customDomain) {
      ...
      if (existingByDomain && existingByDomain._id !== args.tenantId) {
        throw new Error("That custom domain is already linked to another tenant.");
      }
    }
